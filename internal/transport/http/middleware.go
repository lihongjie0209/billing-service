...
import (
	...
	appLimit "github.com/lihongjie0209/billing-service/internal/ratelimit"
...
func Authentication(service *auth.Service, logger *slog.Logger, cfg config.Auth) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		if cfg.PSK.Enabled && auth.MatchesAny(c.FullPath(), cfg.PSK.HTTPPaths) {
			...
			c.Set("subject", "psk")
			ctx := platformprincipal.WithContext(c.Request.Context(), platformprincipal.Principal{ID: "billing-service:psk", Type: platformprincipal.TypeServiceAccount})
...
func billingHTTPRequirement(route string) (platformauthz.Requirement, bool) {
	requirements := map[string]platformauthz.Requirement{
		...
		"/api/v1/payments/refunds/list":     {Resource: "billing.payment", Action: "list", Scope: platformauthz.ScopePrincipal},
	}
