...
func (unavailableUsageReader) Total(context.Context, string, string, string, time.Time, time.Time) (int64, error) {
	return 0, fmt.Errorf("metering gRPC upstream is not configured")
}

func (c *MeteringClient) Total(ctx context.Context, tenantID, applicationID, meterCode string, start, end time.Time) (int64, error) {
	response, err := c.client.QueryUsage(ctx, &meteringv1.QueryUsageRequest{TenantId: tenantID, ApplicationId: applicationID, MeterCode: meterCode, StartAt: timestamppb.New(start), EndAt: timestamppb.New(end), Granularity: "total", Page: &commonv1.PageRequest{Page: 1, PageSize: 1}})
	if err != nil {
		return 0, err
	}
