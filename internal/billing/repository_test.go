...
func TestSQLRepository_GetSubscriptionScansNullPendingPlan(t *testing.T) {
	...
	repository := &SQLRepository{db: db}
	now := time.Date(2026, time.August, 31, 10, 0, 0, 0, time.FixedZone("UTC+8", 8*60*60))
	query := "SELECT " + subscriptionColumns + " FROM subscriptions WHERE tenant_id=? AND id=?"
	mock.ExpectQuery(regexp.QuoteMeta(query)).WithArgs("tenant-1", "subscription-1").WillReturnRows(
		sqlmock.NewRows([]string{"id", "tenant_id", "plan_id", "status", "current_period_start", "current_period_end", "cancel_at_period_end", "canceled_at", "external_reference", "pending_plan_id", "pending_change_at", "version", "created_at", "updated_at", "created_by", "updated_by"}).
			AddRow("subscription-1", "tenant-1", "plan-1", "active", now, now.AddDate(0, 1, 0), false, nil, "", nil, nil, int64(1), now, now, "user-1", "user-1"),
	)
	mock.ExpectClose()

	value, err := repository.GetSubscription(context.Background(), "tenant-1", "subscription-1")
	if err != nil {
