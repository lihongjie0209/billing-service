...
func newApplicationVerifier(cfg config.Config, registry *outbound.Registry) (appaccess.Verifier, error) {
	...
	if registry == nil {
		return nil, errors.New("billing service requires outbound registry")
	}
	connection, ok := registry.GRPC("application")
	if !ok {
		return nil, errors.New("billing service requires outbound.grpc.application")
	}
	return appaccess.NewGRPCVerifier(applicationv1.NewApplicationServiceClient(connection), 2*time.Second), nil
}

var BillingModule = fx.Module("billing", fx.Provide(billing.NewRepository, billing.NewMeteringClient, billing.NewPaymentGateway, newApplicationVerifier, billing.NewRuntimeService))
